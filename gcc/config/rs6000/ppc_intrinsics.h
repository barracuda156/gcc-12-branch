 ...
 * - Some of the intrinsic definitions use the "volatile" specifier
 *   on the "asm" statements in order to work around what appears
 *   to be a bug in the compiler/optimizer. In general we have avoided
 *   the use of "volatile" because it suppresses optimization
 *   on the generated instructions. The instructions to which "volatile"
 *   has been added where it appears that it should not be needed
 *   are lhbrx and lwbrx.
 *
 ...
 *
 * We don't mention "%1" below: operand[1] needs to be skipped
 * as it's just a placeholder to let the compiler know that rA is read
 ...
 *
 * Please see Motorola's "The Programming Environments
 * for 32-Bit Microprocessors" for a description of what these do.
 *
 *   Parameter descriptions:
 *
 *     base             Starting address for figuring out
 *                      where the cacheline is.
 *
 *     index            Byte count to be added to the base address
 *                      for purposes of calculating the effective address
 *                      of the cacheline to be operated on.
 *                                      
 *   Effective Address of cacheline to be manipulated =
 *     (char*) base + index
 *      
 *   WARNING: The size and alignment of cachelines are subject
 *     to change on future processors! Cachelines are 32 bytes
 *     in size and are aligned to 32 bytes on PowerPC 601, 603,
 *     604, 750, 7400, 7410, 7450 and 7455.
 *
 */
 ...
 *
 * Convert the input value to a signed long and place in the low 32 bits
 * of the FP register. Clip to LONG_MIN or LONG_MAX if the FP
 ...
 * destination register.  Clip to LLONG_MIN (-2**63) or LLONG_MAX (2**63-1)
 * if the FP value exceeds the range representable by a int64_t.
 * Use the rounding mode indicated in the FPSCR.
 * 
 ...
 *
 * Double precision arguments are used to prevent the compiler
 * from issuing frsp instructions upstream.
 */
 ...
 * __fsels - Floating Select (Single-Precision variant)
 *
 * An artificial single precision variant of fsel. This produces
 * the same results as fsel, but is useful because the result is cast
 * as a float, discouraging the compiler from issuing a frsp instruction
 * afterward.
 */
 ...
 * __fsqrt - Floating-Point Square Root (Double-Precision)
 *
 * WARNING: Illegal instruction for PowerPC 603, 604, 750, 7400, 7410, 7450 and 7455
 */
